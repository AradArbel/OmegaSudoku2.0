    ...
    public class SudokuSolver
    {
        ...
        public static bool findEmptyPlace(SudokuBoard board)
        {
            ...
            for (row = 0; row < Constants.boardRange; row++)
                for (col = 0; col < Constants.boardRange; col++)
                    if (board.Board[row, col].Value == 0) //marked with 0 is empty
                        return true;
            return false;
        }
        ...
        public static bool isValidPlace(SudokuCell cell, SudokuBoard board, int num)
        {
            ...
            return !isExistInRow(cell, board, num)
                && !isExistInCol(cell, board, num)
                && !isExistInBox((int)(cell.Row - cell.Row % Constants.boxRange), (int)(cell.Col - cell.Col % Constants.boxRange), board, num);
        }
        ...
        public static bool solveSudoku(SudokuBoard board, int row, int col)
        {
            ...
            for (int num = Constants.minCellValue; num <= Constants.maxCellValue; num++)
            {
                if (isValidPlace(board.Board[row, col], board, num))
